/* Harnais de validation — Canucks de Vancouver (jsdom) */
const fs = require('fs');
...
/* Constantes propres à l'équipe — alignement publié sur ushl.ca (TeamRosters VANCOUVER,
   relevé du 11 septembre 2026 via data/rosters.json).
   masseSousContrat = somme des salaires CT>0 de l'alignement, moins Caleb
   Desnoyers (975 000 $, au club-école : jamais compté au plafond tant qu'il
   n'est pas prolongé). Aucun contrat échu à ce relevé. */
const ATTENDU = {
  equipe: 'VANCOUVER',
  taille: 22,
  echantillon: {nom: 'Joseph Veleno', salaire: 9075000, ct: 2, ov: 82},
  masseSousContrat: 92675000,
  nbSousContrat: 21,
  masseTotale: 93650000
};
...
(async () => {
  ...
  console.log('— Moteur de profils (tableaux 10-11-12) et matrices associées — fixtures vérifiées à la main (recote du 28 juin — joueurs des Sharks conservés comme références)');
  ...
  const fixtureXtra = [
    ...
    '    [Jaden Schwartz](https://x/#Jaden Schwartz) VANCOUVER         C   82  25  69  94 187  10 1767  81 2.28',
    '    [Alex Tuch](https://x/#Alex Tuch) ANAHEIM         RW  77  27  32  59 183  16 1503 243 2.38',
    '    [* Backup_RW](https://x/#Backup_RW) VANCOUVER         LW   3   0   0   0   0   0    0   0 0.00'
  ].join('\n');
  const x = S.parseXtra(fixtureXtra, 'VANCOUVER');
  egal(x.length, 2, 'Deux patineurs VANCOUVER extraits (Anaheim exclu)');
  ...
  const enAlignement = S.SECOURS_ROSTER.filter(x => !x.horsAlignement);
  egal(rangees.length, enAlignement.length, enAlignement.length + ' rangées dans la table d\'alignement (club-école à part)');
  egal(doc.querySelectorAll('#tableHorsAlign tbody tr').length, 1, 'Caleb Desnoyers dans la table «Hors alignement»');
  ...
  const comptabilises = actifs.filter(x2 => x2.ct > 0 && !x2.horsAlignement);
  ...
  egal(masse, ATTENDU.masseSousContrat, 'Masse des salaires sous contrat = alignement VANCOUVER du 11 septembre (club-école exclu)');
  egal(actifs.reduce((s2, x2) => s2 + x2.salaire, 0), ATTENDU.masseTotale,
    'Masse totale du bloc VANCOUVER (contrats échus compris)');
  egal(actifs.filter(x2 => x2.ct === 0).length, 0, 'Aucun contrat échu au relevé du 11 septembre');
  egal(actifs.filter(x2 => x2.horsAlignement).length, 1, 'Un joueur au club-école (Caleb Desnoyers)');
  ...
  console.log('— Composition d\'équipe : signés + prolongés, retraits, ajouts, décomptes');
  {
    ...
    egal(base.length, S.SECOURS_ROSTER.filter(x=>!x.backup && x.ct>0 && !x.horsAlignement).length,
      'La composition démarre avec tous les joueurs signés de l\'alignement (club-école à part)');
    ok(base.every(j=>j.prov==='signe'), 'Provenance «signé» pour les contrats en cours');
    const somme = base.reduce((a,j)=>a+j.salaire,0);
    const attendu = S.SECOURS_ROSTER.filter(x=>!x.backup && x.ct>0 && !x.horsAlignement).reduce((a,j)=>a+j.salaire,0);
  ...
  console.log('— Parseur de la page «Finances» (contrats à jour, dont les 0 an)');
  {
    const fxFin = `<html><body>
      <p>VANCOUVER 0-0-0 (12e Western)</p>
  ...
  egal(selEqOvd.value, 'VANCOUVER', 'Vancouver sélectionnée par défaut');
  egal(selEqOvd.querySelector('option').value, 'VANCOUVER', 'Vancouver en tête de liste');
  ...
  // retour à Vancouver : la liste redevient celle de l'alignement
  selEqOvd.value = 'VANCOUVER';
  selEqOvd.dispatchEvent(new W.Event('change'));
  egal(selOvd.querySelectorAll('option').length, nbPatineurs + 1,
    'Retour à Vancouver : la liste redevient celle de l\'alignement');
  ...
  const vBel = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Kieffer Bellows'), 'UFA', 1);
  egal(vBel.code, 'surpaye', 'Kieffer Bellows (AD, OV 83, 28 ans, 10,5 M, UFA) → Surpayé');
  egal(vBel.minEff, 8750000, 'Charte Bellows : 8 750 000 $');
  egal(vBel.ecart, 1750000, 'Écart Bellows : +1 750 000 $');
  const vTip = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Owen Tippett'), 'RFA', 1);
  egal(vTip.code, 'aubaine', 'Owen Tippett (AD, OV 84, 27 ans, 10,625 M, RFA) → Aubaine salariale');
  egal(vTip.ecart, 1625000, 'Écart Tippett : −1 625 000 $ sous la charte (12,25 M)');
  const vSal = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Elias Salomonsson'), 'RFA', 1);
  egal(vSal.code, 'aubaine', 'Elias Salomonsson (OV 77, 22 ans, 800 k, RFA) → Aubaine salariale');
  egal(vSal.ecart, 150000, 'Écart Salomonsson : −150 000 $ sous la charte (950 k)');
  ...
  const vBed = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Jan Bednar'), 'RFA', 1);
  egal(vBed.minEff, 2250000, 'Jan Bednar (G, OV 79, 24 ans) évalué à l\'échelon 78 : 2 250 000 $');
  egal(vBed.code, 'aubaine', 'Jan Bednar à 700 k → Aubaine salariale');
  const vDos = S.verdictCharte(S.ETAT.roster.find(j=>j.nom==='Lukas Dostal'), 'UFA', 1);
  egal(vDos.minEff, 3500000, 'Lukas Dostal (G, OV 80, 26 ans) évalué à l\'échelon 79 : 3 500 000 $');
  egal(vDos.code, 'aubaine', 'Dostal à 3,125 M → Aubaine salariale');
  ...
  ok(S.SECOURS_ROSTER.every(j=>!j.backup),
    'Aucun joueur de remplacement dans le bloc VANCOUVER du fichier .ros');
  ...
  selJoueur.value = 'Joseph Veleno';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('surpaye'), 'Veleno sélectionné : badge Surpayé');
  ...
  egal(celluleActive.textContent.replace(/[\s\u00a0\u202f]/g,''), '8750$',
    'Cellule active : 8 750 $ (RFA 5 ans max 26 ans, OV 82)');
  egal(doc.getElementById('csStatut').value, 'RFA', 'Charte RFA proposée d\'après l\'âge (26 ans)');
  selJoueur.value = 'Dougie Hamilton';
  selJoueur.dispatchEvent(new W.Event('change'));
  egal(doc.getElementById('csStatut').value, 'UFA', 'Charte UFA proposée d\'après l\'âge (33 ans)');
  ...
  selJoueur.value = 'Jan Bednar';
  selJoueur.dispatchEvent(new W.Event('change'));
  ok(doc.getElementById('csVerdict').classList.contains('aubaine'), 'Jan Bednar : badge Aubaine salariale');
  ...
  W.localStorage.removeItem('van_resignatures_v1');
  W.localStorage.removeItem('van_transac_v1');
  ...
  const sjLigue = S.ligueJoueurs('VANCOUVER');
  egal(sjLigue.length, ATTENDU.taille, 'Le bloc VANCOUVER compte ' + ATTENDU.taille + ' joueurs');
  tableauEgal(sjLigue.map(j=>j.nom).sort(), S.SECOURS_ROSTER.map(j=>j.nom).sort(),
    'Le bloc VANCOUVER et la formation de secours listent les mêmes joueurs');
  ok(sjLigue.every(j=>{
  ...
  const gDos = S.SECOURS_ROSTER.find(j=>j.nom==='Lukas Dostal');
  egal(S.ovGardien(gDos).arrondi, gDos.ov, 'Dostal : OV recalculé = OV stocké');
  const gBed = S.SECOURS_ROSTER.find(j=>j.nom==='Jan Bednar');
  egal(S.ovGardien(gBed).arrondi, 78,
    'Bednar : la formule estimée donne 78 (ushl.ca publie 79 au relevé du 11 septembre — écart d\'un point documenté)');
  ...
  egal(vide.sj.masseAvant, ATTENDU.masseSousContrat, 'Masse de départ des Canucks');
  ...
  const ech1 = S.txCalculer({partenaire:'DALLAS', sj:['Kieffer Bellows'], part:['Evan Bouchard']});
  egal(ech1.sortSJ.length, 1, 'Un joueur cédé par Vancouver');
  egal(ech1.sortPart.length, 1, 'Un joueur cédé par Dallas');
  egal(ech1.sj.masseApres, 97675000, 'Vancouver : 92 675 000 − 10 500 000 + 15 500 000');
  egal(ech1.part.masseApres, 83250000, 'Dallas : 88 250 000 − 15 500 000 + 10 500 000');
  egal(ech1.sj.margeApres, 104000000 - 97675000, 'Marge des Canucks après l\'échange');
  egal(ech1.sj.nApres, ech1.sj.nAvant, 'Un pour un : effectif inchangé');
  tableauEgal(ech1.sj.effectif, {C:5, AG:4, AD:2, D:9, G:2}, 'Effectif des Canucks après l\'échange');
  ok(!ech1.sj.depasse && !ech1.part.depasse, 'Les deux équipes restent sous le plafond');
  ok(ech1.sj.apres.some(j=>j.nom==='Evan Bouchard'), 'Le joueur acquis figure dans l\'effectif des Canucks');
  ok(!ech1.sj.apres.some(j=>j.nom==='Kieffer Bellows'), 'Le joueur cédé n\'y figure plus');

  const trop = S.txCalculer({partenaire:'DALLAS', sj:[], part:['Evan Bouchard']});
  egal(trop.sj.masseApres, 108175000, 'Acquisition sèche de 15,5 M : 108 175 000 $');
  ok(trop.sj.depasse, 'Dépassement du plafond de 104 M$ détecté');
  ok(!trop.part.depasse, 'Dallas, qui se dégage, reste conforme');

  const nomInconnu = S.txCalculer({partenaire:'DALLAS', sj:['Personne'], part:[]});
  egal(nomInconnu.sortSJ.length, 0, 'Un nom introuvable est ignoré sans planter');

  console.log('— Avertissements d\'effectif');
  const avDal = S.txAvertissements(vide.part);
  ok(avDal.some(a=>a.includes('gardien')), 'Dallas n\'a qu\'un gardien : avertissement');
  egal(S.txAvertissements(vide.sj).length, 0, 'Vancouver : effectif complet, aucun avertissement');
  ...
  const ham = S.SECOURS_ROSTER.find(j=>j.nom==='Dougie Hamilton');
  proche(S.txOvPrecis(ham), 81.80, 0.01, 'OV décimal d\'un défenseur');
  egal(Math.round(S.txOvPrecis(ham)), ham.ov, 'L\'OV décimal s\'arrondit sur l\'OV affiché');
  const dost = S.SECOURS_ROSTER.find(j=>j.nom==='Lukas Dostal');
  egal(Math.round(S.txOvPrecis(dost)), dost.ov, 'OV décimal d\'un gardien via la formule estimée');
  egal(S.txOvPrecis(null), null, 'Aucun joueur : aucun OV');

  egal(S.txJoueurDe('VANCOUVER', 'Dougie Hamilton').nom, 'Dougie Hamilton', 'Joueur retrouvé dans son club');
  egal(S.txJoueurDe('DALLAS', 'Evan Bouchard').ov, 84, 'Joueur retrouvé chez le partenaire');
  egal(S.txJoueurDe('DALLAS', 'Dougie Hamilton'), null, 'Joueur absent de l\'équipe demandée');
  egal(S.txJoueurDe('', 'Dougie Hamilton'), null, 'Aucune équipe : aucun joueur');

  Object.assign(S.TX_ETAT, {partenaire:'DALLAS', sj:[], part:[]});
  egal(S.txRoleJoueur('VANCOUVER'), 'sj', 'Un joueur des Canucks se cède');
  egal(S.txRoleJoueur('DALLAS'), 'part', 'Un joueur du partenaire s\'acquiert');
  egal(S.txRoleJoueur('BOSTON'), null, 'Une équipe tierce n\'entre pas dans l\'échange');
  S.TX_ETAT.sj = ['Dougie Hamilton'];
  ok(S.txDansEchange('VANCOUVER', 'Dougie Hamilton'), 'Joueur déjà dans l\'échange');
  ok(!S.txDansEchange('VANCOUVER', 'Owen Tippett'), 'Joueur absent de l\'échange');
  ok(!S.txDansEchange('BOSTON', 'Dougie Hamilton'), 'Hors des deux clubs : jamais dans l\'échange');
  S.TX_ETAT.sj = [];
  ...
  egal(cmp1.paires.length, 1, 'Bellows contre Bouchard : un duel');
  ...
  egal(cmp1.ecart.salaire, 15500000 - 10500000, 'Écart de masse échangée : +5 000 000 $');
  const cmpN = S.txComparatif(S.txCalculer({partenaire:'DALLAS', sj:['Dougie Hamilton','Simon Edvinsson'], part:['Evan Bouchard']}));
  proche(cmpN.moyCede.ov, 82, 0.001, 'Moyenne d\'OV de deux joueurs cédés');
  egal(cmpN.moyCede.salaire, 7500000 + 8750000, 'Les salaires s\'additionnent au lieu de se moyenner');
  const gard = S.txComparatif(S.txCalculer({partenaire:'DALLAS', sj:['Lukas Dostal'], part:['Evan Bouchard']}));
  ...
  egal(res[0].nom, 'Canucks de Vancouver', 'Premier résumé : Vancouver');
  ...
  egal(res[0].apres.salaire, 97675000, 'Masse au plafond après l\'échange');
  ...
  ok(!Array.from(selTx.options).some(o=>o.value==='VANCOUVER'), 'Vancouver absent de la liste des partenaires');
  egal(selTx.options[1].textContent, 'Anaheim', 'Le menu ne donne que le nom de la ville');
  ...
  egal(doc.getElementById('txEqA').value, 'VANCOUVER', 'Équipe A par défaut : mon club');
  egal(doc.getElementById('txEqB').value, 'DALLAS', 'Équipe B suit l\'équipe partenaire');
  ok(Array.from(doc.getElementById('txEqA').options).some(o=>o.textContent.includes('mon club')),
    'Vancouver est identifié comme mon club');
  egal(doc.getElementById('txJoA').options.length, ATTENDU.taille, 'Joueur A : tout l\'effectif des Canucks');
  ...
  chg('txJoA', 'Kieffer Bellows');
  chg('txJoB', 'Evan Bouchard');
  const fiches = doc.querySelectorAll('#txDuel .duel-fiche');
  egal(fiches.length, 2, 'Deux fiches de joueur');
  ok(fiches[0].textContent.includes('Kieffer Bellows'), 'Fiche A : le joueur choisi');
  ok(fiches[1].textContent.includes('Evan Bouchard'), 'Fiche B : le joueur choisi');
  ok(fiches[0].textContent.includes('Vancouver'), 'La ville figure sur la fiche');
  ok(fiches[0].textContent.includes('83,17'), 'OV décimal affiché sur la fiche');
  ok(fiches[0].textContent.includes('Power Forward'), 'Profil USHL affiché sur la fiche');
  ...
  ok(l1.querySelector('.duel-jauge.a').classList.contains('gagne'), 'Bellows gagne IN (83 contre 70)');
  ok(!l1.querySelector('.duel-jauge.b').classList.contains('gagne'), 'Bouchard ne gagne pas IN');
  const lSK = doc.querySelectorAll('#txDuel .duel-ligne')[6];
  ok(lSK.querySelector('.duel-jauge.b').classList.contains('gagne'), 'Bouchard gagne SK (85 contre 80)');
  ...
  ok(boutons()[0].textContent.includes('cédés'), 'Joueur des Canucks : ajout aux joueurs cédés');
  ok(boutons()[1].textContent.includes('reçus'), 'Joueur du partenaire : ajout aux joueurs reçus');
  boutons()[0].click();
  boutons()[1].click();
  tableauEgal(S.litTransac().sj, ['Kieffer Bellows'], 'Le joueur cédé est enregistré');
  ...
  ok(cotes[0].textContent.includes('Vancouver cède'), 'Colonne de Vancouver');
  ok(cotes[1].textContent.includes('Dallas cède'), 'Colonne du partenaire');
  egal(cotes[0].querySelectorAll('.ech-j').length, 1, 'Un joueur du côté de Vancouver');
  ok(cotes[0].textContent.includes('Kieffer Bellows'), 'Le joueur cédé y figure');
  ok(cotes[0].textContent.includes('OV 83'), 'Son OV y figure');
  ok(cotes[0].querySelector('.ech-total').textContent.includes('10,5 M'), 'Masse cédée par Vancouver');
  ...
  ok(doc.querySelector('#txBilan').textContent.replace(/[\s\u00a0\u202f]/g,'').includes('97675000'),
    'Masse des Canucks après l\'échange : 97 675 000 $');
  ...
  egal(tab.querySelectorAll('tbody.tx-paire').length, 3,
    'Trois blocs : ce qui change de mains, Vancouver, le partenaire');
  ok(tab.textContent.includes('Ce qui change de mains'), 'Bloc de l\'échange');
  ok(tab.textContent.includes('Canucks de Vancouver'), 'Bloc de Vancouver');
  ok(tab.textContent.includes('Stars de Dallas'), 'Bloc du partenaire');
  ...
  ok(som.includes('Kieffer Bellows') && som.includes('Evan Bouchard'), 'Le sommaire nomme les deux joueurs');
